...
local bomb_crack_time = manolis.popcorn.config.bombDefuseTime

if SERVER then
	util.AddNetworkString("BombCracker_Hold")
	util.AddNetworkString("BombCracker_Sparks")
end

if CLIENT then
	SWEP.PrintName = DarkRP.getPhrase('terror_defuser')
	SWEP.Slot = 4
	SWEP.SlotPos = 1
	SWEP.DrawAmmo = false
...
SWEP.Author = "Willox & Manolis Vrondakis"
SWEP.Instructions = DarkRP.getPhrase('terror_instructions')
SWEP.Contact = ""
SWEP.Purpose = ""
...
function SWEP:Initialize()
	...
	if SERVER then
		net.Start("BombCracker_Hold")
			net.WriteEntity(self)
			net.WriteBit(true)
		net.Broadcast()

		self:SetCrackTime( bomb_crack_time )
	end
end	
...
function SWEP:PrimaryAttack()
	...
	if IsValid(ent) and tr.HitPos:Distance(self.Owner:GetShootPos()) <= 50 and ent.IsBomb and ent.GetArmed and ent:GetArmed()>0 then
...
function SWEP:Succeed()
	...
	if SERVER and IsValid(ent) and tr.HitPos:Distance(self.Owner:GetShootPos()) <= 50 and ent.IsBomb then
		ent:Defuse(self.Owner)
...
function SWEP:Think()
	...
	if self.IsCracking and IsValid(self.Owner) then
		...
		if not IsValid(tr.Entity) or tr.HitPos:Distance(self.Owner:GetShootPos()) > 50 or not tr.Entity.IsBomb then
			self:Fail()
...
if(CLIENT) then
	...
	function SWEP:DrawHUD()
		if self.IsCracking then
			...
			local fontw, fonth = surface.GetTextSize(DarkRP.getPhrase('terror_defusing'))
			local fontx, fonty = (x + (w / 2)) - (fontw / 2), (y + (h / 2)) - (fonth / 2)
			
			surface.SetTextPos(fontx + 1, fonty+1)
			surface.SetTextColor(color_black)
			surface.DrawText(DarkRP.getPhrase('terror_defusing')..dots)
	...
	net.Receive("BombCracker_Hold", function()
		local ent = net.ReadEntity()
		local state = (net.ReadBit() == 1)

		if IsValid(ent) and ent:IsWeapon() and ent:GetClass():lower() == "weapon_bombdefuse" and not game.SinglePlayer() and ent.SetWeaponHoldType then
			if not state then
	...
	net.Receive("BombCracker_Sparks", function()
		local ent = net.ReadEntity()
