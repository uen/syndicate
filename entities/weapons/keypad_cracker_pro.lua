...
local keypad_crack_time = manolis.popcorn.config.proKeypadCrackTime
...
if(CLIENT) then
	...
	net.Receive("KeypadCracker_Hold", function()
		...
		if IsValid(ent) and ent:IsWeapon() and ent:GetClass():lower() == "keypad_cracker_pro" and not game.SinglePlayer() and ent.SetWeaponHoldType then
			if not state then
