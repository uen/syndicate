...
function ENT:Initialize()
	...
	self.Damage = 100*3e5*self.YieldSlow
	self.Duration = 10
...
function ENT:Think()
	...
	local FTime = CurrentTime - (self.lastThink or 0)
		...
		if found:IsValid() then
			...
			if found:IsNPC() then
				...
				if self:LOS(found,entpos) then
					...
					util.BlastDamage(self.Weapon, self, entpos, 8, self.Damage*RadIntensity*entdist)
			...
			elseif found:IsPlayer() then
				...
				if self:LOS(found,entpos) then
					entdist = ((entpos - self.Pos):Length())^-2
					found:TakeDamage(self.Damage*RadIntensity*entdist, self)
