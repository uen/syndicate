...
function meta:keysOwn(ply)
    ...
    if ply.OwnedNumz == 0 then
        timer.Create(ply:SteamID64() .. "propertytax", manolis.popcorn.config.propertyTaxTime, 0, function() ply.doPropertyTax(ply) end)
    end
...
function pmeta:initiateTax()
    ...
    local uniqueid = self:SteamID64() -- so we can destroy the timer if the player leaves
...
local function OwnDoor(ply)
    ...
    if trace.Entity:isKeysOwnedBy(ply) then
        if(trace.Entity:IsVehicle()) then
            DarkRP.notify(ply,1,4,'You cannot sell vehicles')
            return
        end
        ...
        for k,v in pairs(manolis.popcorn.buildings.buildings) do
            if(v.doors) then
                for k2,v2 in pairs(v.doors) do
                    if(v2.id==trace.Entity:doorIndex()) then
                        building = v
                    end
                end
            end
        ...
        if(building) then
            ...
            for k,v in pairs(building.doors) do
                DarkRP.doorIndexToEnt(v.id):keysUnOwn(ply)
                DarkRP.doorIndexToEnt(v.id):setKeysTitle(nil)

                hook.Call("playerKeysSold", GAMEMODE, ply, DarkRP.doorIndexToEnt(v.id), 0)   
            end
        else
            DarkRP.notify(ply,0,4,'You have sold this door')
        ...
        if(ply and table.HasValue(manolis.popcorn.config.bannedBuildingPurchase, ply:Team())) then
            DarkRP.notify(ply,1,4,'You cannot capture this building')
            return
        end
        ...
        local iCost = hook.Call("get" .. ("Door") .. "Cost", GAMEMODE, ply, trace.Entity)
        if(building) then
            iCost = building.rent or manolis.popcorn.config.defaultRent
        end
        ...
        if(building) then
            for k,v in pairs(building.doors) do
                DarkRP.doorIndexToEnt(v.id):keysOwn(ply)
                hook.Call("playerBoughtDoor", GAMEMODE, ply, DarkRP.doorIndexToEnt(v.id))   
            end

            if not bSuppress then
                DarkRP.notify(ply, 0, 4,DarkRP.getPhrase('building_buy', building.name, DarkRP.formatMoney(iCost)))
            end

            building.ownedBy = ply
        else
            if not bSuppress then
                DarkRP.notify(ply, 0, 4,DarkRP.getPhrase('building_doorbuy', DarkRP.formatMoney(iCost)))
            end
        end
    end
...
local function AddDoorOwner(ply, args)
    ...
    if(building) then
        for k,v in pairs(building.doors) do
            DarkRP.doorIndexToEnt(v.id):addKeysAllowedToOwn(target)
            hook.Call("onAllowedToOwnAdded", nil, ply, DarkRP.doorIndexToEnt(v.id), target)  
        end
    end
