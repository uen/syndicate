...
function meta:drawOwnableInfo()
    ...
    local title = self.buildingInfo and self.buildingInfo.name
    ...
    local x, y = ScrW() / 2, ScrH() / 2 + 10
    draw.DrawNonParsedText(table.concat(doorInfo, "\n"), "manolisDoorDisplayFont", x+2, y + 2 , black, 1)
    draw.DrawNonParsedText(table.concat(doorInfo, "\n"), "manolisDoorDisplayFont", x, y, (blocked or owned) and white or manolis.popcorn.config.promColor, 1)
end
